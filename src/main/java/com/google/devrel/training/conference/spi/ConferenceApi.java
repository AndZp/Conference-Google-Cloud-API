 ...
 */
@Api(name = "conference", version = "v1", scopes = { Constants.EMAIL_SCOPE }, clientIds = { Constants.WEB_CLIENT_ID, Constants.API_EXPLORER_CLIENT_ID, Constants.ANDROID_CLIENT_ID }, audiences = {
		Constants.ANDROID_AUDIENCE }, description = "API for the Conference Central Backend application.")
public class ConferenceApi {
	...
	private static Profile getProfileFromUser(User user) {
		...
		if (profile == null) {
			...
			profile = new Profile(getUserId(user), extractDefaultDisplayNameFromEmail(email), email, TeeShirtSize.NOT_SPECIFIED);
		}
		return profile;
	}
	...
	public Conference createConference(final User user, final ConferenceForm conferenceForm) throws UnauthorizedException {
		...
		Conference conference = ofy().transact(new Work<Conference>() {
			...
			public Conference run() {
				...
				queue.add(ofy().getTransaction(), TaskOptions.Builder.withUrl("/tasks/send_confirmation_email").param("email", profile.getMainEmail()).param("conferenceInfo", conference.toString()));
				return conference;
			}
		});
		return conference;
	...
	public List<Conference> filterPlayground() {
		...
		/*
		 * // Filter on city query = query.filter("city =", "London"); // query
		 * = query.filter("city =", "Default City");
		 * 
		 * // Add a filter for topic = "Medical Innovations" query =
		 * query.filter("topics =", "Medical Innovations");
		 * 
		 * // Add a filter for maxAttendees query = query.filter(
		 * "maxAttendees >", 8); query = query.filter("maxAttendees <",
		 * 10).order("maxAttendees").order("name");
		 * 
		 * // Add a filter for month {unindexed composite query} // Find
		 * conferences in June query = query.filter("month =", 6);
		 */

		// multiple sort orders

		query = query.filter("city =", "Tokyo").filter("seatsAvailable <", 10).filter("seatsAvailable >", 0).order("seatsAvailable").order("name").order("month");
	...
	public WrappedBoolean registerForConference(final User user, @Named("websafeConferenceKey") final String websafeConferenceKey) throws UnauthorizedException, NotFoundException, ForbiddenException, ConflictException {
		// If not signed in, throw a 401 error.
		if (user == null) {
			throw new UnauthorizedException("Authorization required");
		}
	...
	public WrappedBoolean unregisterFromConference(final User user, @Named("websafeConferenceKey") final String websafeConferenceKey) throws UnauthorizedException, NotFoundException, ForbiddenException, ConflictException {
		// If not signed in, throw a 401 error.
		if (user == null) {
			throw new UnauthorizedException("Authorization required");
		}
	...
	private static String getUserId(User user) {
		...
		if (userId == null) {
			// LOG.info("userId is null, so trying to obtain it from the
			// datastore.");
