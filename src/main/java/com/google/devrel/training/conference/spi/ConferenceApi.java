...
public class ConferenceApi {
	...
	// @ApiMethod(name = "filterPlayground", path = "filterPlayground",
	// httpMethod = HttpMethod.GET)
	public List<Conference> filterPlayground() {
		Query<Conference> query = ofy().load().type(Conference.class).filter("city =", "London").filter("seatsAvailable >", 0).filter("seatsAvailable <", 30).order("seatsAvailable").order("month");

		// Filter on city
		/*
		 * query = query.filter("city =", "London"); query.filter("topics =",
		 * "Medical Innovations"); query = query.filter("month =", 1);
		 */
		/*
	...
	private String getUserId(final User user) {
		return (getProfileFromUser(user)).getUserId();
	}
